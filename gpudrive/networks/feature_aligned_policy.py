...
class FeatureAlignedPolicy(
    ...
    def __init__(
        ...
        self.feature_aligned_embed = nn.Sequential(
            pufferlib.pytorch.layer_init(
            ...
            self.act_func,
            nn.Dropout(self.dropout),
            pufferlib.pytorch.layer_init(nn.Linear(hidden_dim, hidden_dim)),
        )

        self.shared_embed = nn.Sequential(
            nn.Linear(self.input_dim + hidden_dim, self.hidden_dim),
            nn.Dropout(self.dropout),
        )
    ...
    def encode_observations(self, observation):
        ...
        combined = torch.cat([partner_embed, road_map_embed], dim=1)
        feature_aligned_embed = self.feature_aligned_embed(combined)
        
        embed = torch.cat([ego_embed, feature_aligned_embed], dim=1)
